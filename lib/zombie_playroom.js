...
function ZombiePlayeroom (ctx) {
  ...
  this.runnerZombie = new RunnerZombie(500, 200, this.player, 0);
  var that = this;
  this.normalZombies.forEach(function (zombie, idx1){
    for (var idx2 = 0; idx2 < that.normalZombies.length; idx2++) {
      ...
      that.runnerZombie.registerOtherZombie(zombie);
    }
  })

  // this.sampleZombie = new NormalZombie(500, 200, this.player, 0, image);
  this.boneZombie = new BoneZombie(400, 200, this.player, 0, this.wall);
}
...
ZombiePlayeroom.prototype.draw = function () {
  ...
  this.boneZombie.render(this.ctx)
  var that = this
  ...
  // this.boneZombie.doZombieThings();
  this.boneZombie.animate(this.ctx);
  // debugger
  this.runnerZombie.doZombieThings(this.ctx)
  this.runnerZombie.animate(this.ctx);
